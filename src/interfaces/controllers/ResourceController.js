class ResourceController {
  ...
  async updateFacility(req, res) { 
    try { 
      ...
      // Log để debug
      console.log('Update facility - ID:', req.params.id, 'Body:', JSON.stringify(body));
      
      const data = await this.facilityService.update(Number(req.params.id), body); 
      res.status(200).json(data) 
    } catch (e) { 
  ...
  async createClub(req, res) { 
    try { 
      ...
      if (!body.code && body.name) {
        ...
        let baseCode = body.name.toUpperCase().replace(/\s+/g, '').replace(/[^A-Z0-9]/g, '');
      ...
      if (!body.campusId || isNaN(body.campusId)) {
        return res.status(400).json({ message: 'Campus ID là bắt buộc và phải là số hợp lệ' });
      }
      if (!body.code) {
      ...
      if (e.code === 'P2002') {
        return res.status(400).json({ message: 'Mã CLB (code) đã tồn tại. Vui lòng chọn mã khác.' });
      }
      // Kiểm tra nếu lỗi do foreign key
      if (e.code === 'P2003') {
        return res.status(400).json({ message: 'Campus ID hoặc Leader ID không tồn tại.' });
      }
      res.status(500).json({ message: e.message }) 
    } 
  }
  ...
  async assignClubPriority(req, res) { 
    try { 
      ...
      const score = priorityScore !== undefined ? Number(priorityScore) : (priorityLevel !== undefined ? Number(priorityLevel) : 1);
      
      const data = await this.clubService.assignPriority(clubId, Number(facilityId), score, note); 
      res.status(200).json(data) 
    } catch (e) { 
