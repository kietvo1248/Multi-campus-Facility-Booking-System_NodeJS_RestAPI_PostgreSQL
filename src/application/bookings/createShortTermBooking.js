class CreateShortTermBooking {
  ...
  async execute({ userId, userRole, facilityId, date, slots, bookingTypeId, purpose, attendeeCount }) {
    ...
    const conflictApproved = conflicts.find(b => String(b.status).toUpperCase() === 'APPROVED');
    const conflictIdsAll = conflicts
      .filter(b => ['APPROVED', 'PENDING'].includes(String(b.status).toUpperCase()))
      .map(b => b.id);
