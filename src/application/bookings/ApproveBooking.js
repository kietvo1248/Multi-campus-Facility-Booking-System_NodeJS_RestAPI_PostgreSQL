class ApproveBooking {
    ...
    async execute(bookingId, adminId) {
        ...
        if (booking.status !== 'PENDING') throw new Error("Chỉ có thể duyệt các đơn đang chờ (PENDING).");
        ...
        const approvedConflict = await this.bookingRepository.findApprovedConflicts(
            booking.facilityId, booking.startTime, booking.endTime
        );
        if (approvedConflict) {
            throw new Error(`Không thể duyệt. Đã có đơn APPROVED (ID: ${approvedConflict.id}) trong khung giờ này.`);
        }

        // 3. Tìm các đơn PENDING khác bị trùng (Victims)
        const pendingConflicts = await this.bookingRepository.findPendingConflicts(
            booking.facilityId, booking.startTime, booking.endTime, bookingId
        );
